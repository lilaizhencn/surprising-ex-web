...
2. auth refresh/logout、MFA challenge、用户会话撤销和登录历史已有实现；仍需补充设备信任、Passkey 和账户锁定状态接口。
3. 补充独立 user profile、设备管理和账户活动接口；通知查询、帮助文章查询已补入 Gateway。
4. KYC 上传、提交、审核结果、拒绝原因和重新提交已补入 Gateway；仍需合规服务明确异步审核 SLA。
5. REST 已提供行情快照；WebSocket 订阅、重连和事件展示已接入前端，仍需后端公开 sequence、心跳和深度增量恢复契约。
...
10. 划转记录已支持不限定单一产品账户查询并由 Gateway 转发；仍需补充未知结果查询接口，并明确幂等键生命周期。
