...
| 行情 | instrument、candlestick | 已接 REST；支持 favorites、搜索和排序 | candles/depth/trades 已接订阅封装 | WS sequence/恢复 DTO | 同上 |
| 现货交易 | instrument、candles、orders、balances、depth、latest-trade | REST snapshot、下单、开单查询、撤单、余额与手续费估算已接 | candles/depth/trades/private orders 已接订阅封装 | 增量事件映射仍待统一 | 不伪造成交 |
...
| 安全中心 | security scenes/mfa/password/api-keys/sessions/login-history | 状态、高风险写操作、会话撤销和登录历史已接 | 否 | Passkey暂无用户接口 | 否 |
