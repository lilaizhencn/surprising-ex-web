...
| 撤销其他会话 | `POST /api/v1/security/sessions/revoke-all` | Confirmed；Body 必须包含当前 refresh token，仅撤销其他活跃会话 |
...
`POST /api/v1/security/sessions/{sessionId}/revoke` 按当前认证用户和 session id 原子撤销，不通过固定数量的列表查询判断归属。
